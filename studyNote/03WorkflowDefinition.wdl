...
Parameter Metadata{
    这个纯可选部分包含键/值对，其中键是参数名，值是JSON格式的表达式，描述这些参数.
    附加条件：本节中的任何键都必须与工作流输入或输出相对应。03.13.ParameterMetadata.wdl
}
Metadata{
    这个纯可选部分包含用于任何应该存储在工作流中的额外元数据的键/值对。例如，作者或联系电子邮件。3.14.Metadata.wdl
}
Outputs: {
    每个工作流定义都可以指定一个输出部分。这个部分将输出作为Type name =表达式列出，就像任务输出一样。工作流输出也遵循与任务输出相同的语法规则.它们是可以引用所有调用输出、工作流输入、中间值和以前的工作流输出的表达式。例如:03.15.Outputs.wdl{
        注意，它们不能引用调用输入(例如，我们不能使用Int i = t)。i作为工作流输出).但是，这也可以通过将所需的调用输入声明为调用的输出来实现。
    }
    当声明指向分散内部调用的工作流输出时，将使用聚合调用，就像从分散外部引用它的任何表达式一样。03.16.Outputs.wdl{
        在这个例子中t_out是一个Array[String]结果类型，因为调用t是在scatter中
    }
    Omitting Workflow Outputs{
        如果output{…}如果在顶级workfow中省略了，那么工作流引擎应该在其最终输出中包含来自所有调用的所有输出.
        但是，如果希望将工作流作为子工作流调用，则需要使用output块中的表达式来命名和指定输出，就像任务输出一样。这里的基本原理是{
            1 在调用子工作流时显示与调用任务相同的接口
            2 使子工作流的调用者更容易准确地找到调用正在创建的输出。
            3 对于嵌套子工作流，要在顶层给输出一个简单的固定名称，而不是像a.b.c.d out这样的长限定名称.(例如，如果c语言的底层实现发生了变化，哪些可能会发生变化)
        }
    }
}
